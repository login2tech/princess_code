...
class MyOrdersPendingPayment extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isOpen: false,
  ...
  getListings() {
    ...
    fetch(
      endpoint +
        '?action=princess_user_orders&status=Pending_Payment&user_id=' +
  ...
  processPayment(id, amount, obj) {
    ...
    const toHome = NavigationActions.navigate({
      routeName: 'PaymentModeCard',
      params: {
        obj: obj
      },

      onGoBack: () => this.getListings()
    });
    this.props.navigation.dispatch(toHome);
          ...
          <View>
            <ScrollView>
              ...
              >
                ...
                >
                  {this.lang.PENDING_PAYMENT_ORDERS}
                </Text>
              </View>
...
export default connect(mapStateToProps)(MyOrdersPendingPayment);
