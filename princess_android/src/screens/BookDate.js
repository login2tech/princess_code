...
class BookDate extends React.Component {
  ...
  showLocationPicker() {
    RNGooglePlaces.openAutocompleteModal({}, ['placeID'])
    .then(place => {
      alert(place.address);
      // alert(response.address)
        ...
        if (response.didCancel) {
          ...
          this.setState({
            user_location: response.address ? response.address : ''
          });
        }
    })
    ...
    return (
      <View>
        ...
        {this.state.items.map((item, i) => {
          if (!this.state.saloon_data.item_prices[item.name] || this.state.saloon_data.item_prices[item.name] =='no') {
            return false;
          }
          return (
            <View
            ...
            >
              ...
              >
                <Picker.Item
                  label={this.lang.SLOTS + ' ' + (this.lang.CUR_LANG=='en'?item.name : item.name_ar) }
                  value="0"
                />
                <Picker.Item
                  label={'1 x ' + (this.lang.CUR_LANG=='en'?item.name : item.name_ar) }
                  value="1"
                />
                <Picker.Item
                  label={'2 x ' + (this.lang.CUR_LANG=='en'?item.name : item.name_ar) }
                  value="2"
                />
                <Picker.Item
                  label={'3 x ' + (this.lang.CUR_LANG=='en'?item.name : item.name_ar) }
                  value="3"
                />
                <Picker.Item
                  label={'4 x ' + (this.lang.CUR_LANG=='en'?item.name : item.name_ar) }
                  value="4"
                />
                <Picker.Item
                  label={'5 x ' + (this.lang.CUR_LANG=='en'?item.name : item.name_ar) }
                  value="5"
                />
                <Picker.Item
                  label={'6 x ' + (this.lang.CUR_LANG=='en'?item.name : item.name_ar) }
                  value="6"
                />
                <Picker.Item
                  label={'7 x ' + (this.lang.CUR_LANG=='en'?item.name : item.name_ar) }
                  value="7"
                />
                <Picker.Item
                  label={'8 x ' + (this.lang.CUR_LANG=='en'?item.name : item.name_ar) }
                  value="8"
                />
              </Picker>
            </View>
        ...
        {parseInt(obj.go_to_customer_home) && !this.state.visit_provider_location ? (
